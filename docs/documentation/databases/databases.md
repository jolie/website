...
Jolie can be used with various relational/SQL databases, but only with one
instance per program. The API is described here. Please find attached the correct
driver name and the JAR file to place under the `lib` directory (the program's
