init
{
	...
	install( RentalBookFault => nullProcess )
}
...
main
{
	...
	[ rentalBook( request )( response ){ 
		...
		println@Console("! - transaction succeeded. Reservation " + csets.reservationId + " canfirmed!")()
	      }
	}
	
	[ rentalRevbook( request )]{
		println@Console("! - Reversed booking for reservationId: " + request.reservationId )()
	}
	
	[ getRentalPrice( request )( response ){
