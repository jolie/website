...
var css_contentIframe = "#" + contentIframe;
...
var createItems = function ( el, children ){
	...
	$( children ).each( function( i, e ) {
		var li = $( "<li></li>" );
		var a = $( "<a></a>" )
			.attr( "href", root + e.url )
			.attr( "target", contentIframe )
			// every link returns itself to loadMenuItem
			.attr( "onclick", "return loadMenuItem( event );" );
		a.text( e.label );
		$( li ).append( a );
		$( ul ).append( li );
	});
...
var loadMenuItem = function( event ){
	...
	// var doc = el.attr( "href" );
	// $.get( doc, function( data ) {
		$( css_contentIframe ).load(function() {
		$( css_contentIframe ).find( "style" ).remove();
		$( "link" ).each( function( i, link ) {
			$( css_contentIframe ).contents()
				.find( "head" ).append( 
					$( link ).clone().attr( "href",
...
var addTOCToParent = function ( el ) {
	...
	if( par.find( "span" ).text() != "Standard Library API" ){
		...
		$( css_contentIframe ).contents().find( "h2" ).each( function( i, e ) {
			ul.append( $( "<li></li>" )
				.append( $( "<a></a>")
					.attr( "href", "#" + $( e ).attr( "id" ) )
